...
또한, EDA를 진행한 결과, 시술 유형별로 서로 다른 데이터 패턴이 존재함을 확인할 수 있었고, 이를 고려해 패턴에 맞는 방식으로 결측치를 처리하면서 모델 성능을 개선할 수 있었다. 이 경험을 통해 EDA의 중요성을 깨달았다.

또한 도메인 학습을 통해 의미 있는 파생 변수를 설계하고, 변수 특성에 따라 적절한 인코딩 방식과 스케일링 기법을 적용하며 모델링을 진행했다.
