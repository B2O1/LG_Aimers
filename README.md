...
|이름|소감|
|---|---|
|김혜원|난임이라는 낯선 도메인을 처음 접하며 어려움을 느꼈지만, 관련 기사와 영상 자료를 참고하며 배경 지식을 학습함으로써 새로운 도메인 역시 충분한 학습을 통해 보완할 수 있다는 점을 깨달았다.v또한, EDA를 진행한 결과, 시술 유형별로 서로 다른 데이터 패턴이 존재함을 확인할 수 있었고, 이를 고려해 패턴에 맞는 방식으로 결측치를 처리하면서 모델 성능을 개선할 수 있었다. 이 경험을 통해 EDA의 중요성을 깨달았다. 또한 도메인 학습을 통해 의미 있는 파생 변수를 설계하고, 변수 특성에 따라 적절한 인코딩 방식과 스케일링 기법을 적용하며 모델링을 진행했다.v이 과정에서 각 인코딩 및 스케일러 기법의 특징과 적용 목적을 비교·이해할 수 있었으며, 단순히 기법을 사용하는 데 그치지 않고 이론적 근거를 함께 축적할 수 있었다는 점에서 의미 있는 경험이었다.|
...
|
|변해민|난임이라는 생소한 도메인을 분석 대상으로 삼아, 결과에 영향을 미치는 주요 피처들을 탐색하고 해석하는 경험을 할 수 있었다. 이 과정을 통해 사전 지식이 부족한 도메인이라 하더라도 데이터 기반으로 핵심 변수를 파악하고, 점진적으로 이해를 확장해 나가는 분석 접근 방식을 학습할 수 있었다.|